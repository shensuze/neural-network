...
void* func(void* args) {
	...
	for (int i = first; i <= last; i++)
	{
		...
		////输出层的输入值
		//float zout[64][2];
		zout[i][0] = multiple(&wout_1[0], &ahide[i][0], 3) + bout_1;
...
int main()
{
	...
	while (1) {
		...
		MY_ARGS args1 = { 0,7,0 };
		MY_ARGS args2 = { 8,15,0 };
		MY_ARGS args3 = {16,23,0};
		MY_ARGS args4 = {24,31,0};
		MY_ARGS args5 = { 32,39,0 };
		MY_ARGS args6 = { 40,47,0 };
		MY_ARGS args7 = { 48,55,0 };
		MY_ARGS args8 = { 56,63,0 };
		pthread_create(&th1,NULL,func,(void*)&args1);
		pthread_create(&th2,NULL,func,(void*)&args2);
		pthread_create(&th3, NULL, func, (void*)&args3);
		pthread_create(&th4, NULL, func, (void*)&args4);
		pthread_create(&th5, NULL, func, (void*)&args5);
		pthread_create(&th6, NULL, func, (void*)&args6);
		pthread_create(&th7, NULL, func, (void*)&args7);
		pthread_create(&th8, NULL, func, (void*)&args8);
		...
		float c = args1.result + args2.result + args3.result + args4.result+ args5.result + args6.result + args7.result + args8.result;
		//c条件判断
		if (loop > 1 && c - csum_old > -0.01)
		{
			for (int i = 0; i < 12; i++)
			{
				printf("whide_1:%d:%lf\n", i, whide_1[i]);
		...
		csum_old = args1.result + args2.result + args3.result + args4.result + args5.result + args6.result + args7.result + args8.result;
