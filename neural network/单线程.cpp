...
int main()
{
	...
	while(1)
	{
		...
		for (i = 0; i < 64; i++)
		{
			...
			////隐藏层的输入值z
			//float zhide[64][3];
			zhide[i][0] = multiple(whide_1, &x_1[i][0],12)+bhide_1;
			zhide[i][1] = multiple(whide_2, &x_1[i][0],12)+bhide_2;
			zhide[i][2] = multiple(whide_3, &x_1[i][0],12)+bhide_3;
			...
			////输出层的输入值
			//float zout[64][2];
			zout[i][0] = multiple(wout_1, &ahide[i][0], 3) + bout_1;////////////////////////////为什么写*x_1_new不行
			zout[i][1] = multiple(wout_2, &ahide[i][0], 3) + bout_2;///////////////////////////
	...
	for (int i = 0; i < 2; i++)
	{
		////隐藏层的输入值z
			//float zhide[64][3];
		zhide[i][0] = multiple(whide_1, &test[i][0], 12) + bhide_1;
		zhide[i][1] = multiple(whide_2, &test[i][0], 12) + bhide_2;
		zhide[i][2] = multiple(whide_3, &test[i][0], 12) + bhide_3;
